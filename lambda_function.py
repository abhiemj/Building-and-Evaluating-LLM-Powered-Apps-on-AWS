...
def lambda_handler(event, context):
    ...
    elif function == "escalateToSupport":
    ...
    elif function == "getOrderStatus":
        order_id = unpacked.get("orderId", None)
        if order_id:
            statuses = ["Processing", "Shipped", "Delivered", "Delayed", "Cancelled"]
            status = random.choice(statuses)
            responseBody['TEXT']['body'] = f"{{'orderId': '{order_id}', 'status': '{status}'}}"
        else:
            responseBody['TEXT']['body'] = "{'error':'Missing order ID.'}"
    
    elif function == "cancelSubscription":
        custId = unpacked.get("custId", None)
        reason = unpacked.get("reason", "Not provided")

        if custId:
            cancel_id = random.randint(10000, 99999)
            responseBody['TEXT']['body'] = (
                f"{{'cancelId': {cancel_id}, 'custId': '{custId}', 'status': 'Subscription cancelled', 'reason': '{reason}'}}"
            )
        else:
            responseBody['TEXT']['body'] = "{'error':'Customer ID required to cancel subscription.'}"
